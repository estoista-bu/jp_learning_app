...
export function ListeningTestViewer({ words, userId }: ListeningTestViewerProps) {
  ...
  useEffect(() => {
    return () => {
      if (sessionTotal > 0) {
        const prevScore = parseInt(localStorage.getItem(`listening_score_${userId}`) || '0', 10);
        const prevSum = parseInt(localStorage.getItem(`listening_total_${userId}`) || '0', 10);
        localStorage.setItem(`listening_score_${userId}`, (prevScore + sessionCorrect).toString());
        localStorage.setItem(`listening_total_${userId}`, (prevSum + sessionTotal).toString());
      }
    };
